...
namespace Maestria.TypeProviders.Excel
{
    ...
    public class ExcelGenerator : ISourceGenerator
    {
            ...
            foreach (var classSymbol in classSymbols)
            {
                ...
                try
                {
                    context.AddSource(
                        ...
                        SourceText.From(CreateExcelProvider(classSymbol, 1), Encoding.UTF8));
                }
                catch (Exception e)
                {
        ...
        private string CreateExcelProvider(INamedTypeSymbol classSymbol, int worksheetPosition)
        {
            ...
            var columns = GetExcelColumns(attribute.TemplatePath, worksheetPosition);
...
");
    ...
    {{
        public static IEnumerable<{classSymbol.Name}> Load(Stream input, int worksheetPosition = 1)
        {{
            using var workbook = new XLWorkbook(input);
            return Load(workbook, worksheetPosition, null);
        }}

        public static IEnumerable<{classSymbol.Name}> Load(Stream input, string worksheetName)
        {{
            using var workbook = new XLWorkbook(input);
            return Load(workbook, 0, worksheetName);
        }}

        public static IEnumerable<{classSymbol.Name}> Load(string filePath, int worksheetPosition = 1)
        {{
            using var workbook = ExcelExtensions.OpenWorkbook(filePath);
            return Load(workbook, worksheetPosition, null);
        }}

        public static IEnumerable<{classSymbol.Name}> Load(string filePath, string worksheetName)
        {{
            using var workbook = ExcelExtensions.OpenWorkbook(filePath);
            return Load(workbook, 0, worksheetName);
        }}

        public static IEnumerable<{classSymbol.Name}> Load(XLWorkbook workbook, int worksheetPosition = 1, string worksheetName = """")
        {{
            var result = new List<{classSymbol.Name}>();
            var sheet = string.IsNullOrEmpty(worksheetName) ? workbook.Worksheet(worksheetPosition) : workbook.Worksheet(worksheetName);
...
{(isGlobalNamespace ? string.Empty : "}")}");
        ...
        private static IEnumerable<FieldMapInfo> GetExcelColumns(string filePath, int worksheetPosition)
        {
            using var workbook = ExcelExtensions.OpenWorkbook(filePath);
            var sheet = workbook.Worksheet(worksheetPosition);
